...
<center>![An alternative title to this workshop](https://raw.githubusercontent.com/tashapiro/horror-movies/main/images/little-workshop.png){width="60%"}</center>
...
```{r interm-scatter, fig.align="center", message=FALSE}
ggplot(df_slashers, mapping=aes(y=revenue, x=budget))+
  ...
  guides(
    color = guide_legend(override.aes = c(size=4))
  )+
  labs(title="Slasher Movies: Revenue vs. Budget", 
...
df_monthly<-df|>
  ...
  filter(release_year %in% c(2016,2017,2018,2019,2020,2021))
...
ggplot(data = df_monthly_2021, 
       mapping=aes(x=release_month, y=count, group=1))+
  geom_line()+
    labs(x="Month",
...
[**More Scales**]{.underline}**.** We got a taste of scales from our first plot. We can also use scales to modify our x and y axis (e.g. limits, breaks, and labels). We'll need `scale_y_continuous` to change the y axis and `scale_x_discrete` to change our x axis.

[**Annotations**]{.underline}. Sometimes its helpful to call out specific observations in our plots. We can add annotations with ggplot `annotate`.
...
ggplot(data = df_monthly_2021, 
  ...
  annotate(geom="text", 
           label="Horror movies released right in \ntime for Halloween",
           x=10, y=410, color=pal_text, size=3)+
...
ggplot(data = df_monthly, mapping=aes(x=release_month, y=count, group=1))+
  geom_line(color="green")+
...
ggplot(data=df_top_movies, 
  ...
  geom_col(fill = pal_bar, width=0.5)+
...
[**Keep Layering**]{.underline}. Add in budget revenue for comparison. We'll create a smaller `geom_col` within our revenue geom_col.
...
ggplot(data=df_top_movies, mapping=aes(y=reorder(title,revenue), x=revenue))+
  ...
  theme(
    ...
    plot.title=element_text(family="Creepster", size=30, color=pal_title),
